...
export default function SaleForm() {
  ...
  const updateItem = (i: number, field: keyof SaleLineItem, val: string | number) => {
    ...
    if (field === 'productId') {
      ...
      if (prod && prod.retail_price > 0) updated[i].price = prod.retail_price.toString()
    }
    setItems(updated)
  }
  ...
  const outstanding =
    paymentType === 'full' ? 0 :
    paymentType === 'half' ? subtotal - paid : subtotal
  ...
  const handleAddProduct = async () => {
    ...
    try {
      await db.addProduct({
        name: newName.trim(), category, unit_type: newUnit,
        units_per_crate: 12, cost_price: 0, wholesale_price: 0,
        retail_price: parseFloat(newPrice) || 0,
        current_stock: 0, minimum_stock: parseInt(newMinStock) || 5, is_active: true,
      })
      await refresh()
  ...
  if (appState.loading) return <div className="text-muted text-sm p-4">Loading...</div>
  ...
  return (
    <div className="max-w-2xl">
          ...
          {([
            ...
            <button key={p.id} onClick={() => setPaymentType(p.id)}
              className={`flex-1 py-2.5 px-2 rounded-lg border text-sm transition-all cursor-pointer
                ${paymentType === p.id ? 'border-accent bg-accent/10 text-accent' : 'border-border bg-transparent text-soft hover:border-soft'}`}
            >{p.label}</button>
          ))}
        </div>
        {paymentType === 'half' && (
      ...
      <Btn onClick={handleSubmit} size="lg" className="w-full" disabled={saving}>
